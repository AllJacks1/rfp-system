...
export const PrintRequestForPayment = ({ rfp }: { rfp: RFP }) => {
  ...
  const totalQty =
    rfp.line_items?.reduce((sum, item) => sum + (item.qty || 0), 0) || 0;
  const totalDebits =
    rfp.journalEntry
    ...
    <div className="bg-white text-black w-[210mm] min-h-[297mm] mx-auto p-[8mm] box-border text-[8pt] leading-tight flex flex-col font-serif">
      ...
      <header className="border-b-2 border-black pb-2 mb-3 flex justify-between items-end">
        ...
        <div className="text-right">
          <p className="text-[6pt] text-gray-500 uppercase tracking-wider">
            RFP No.
          </p>
          {/* ✅ Use rfp_number for display, id as fallback */}
          <p className="text-[14pt] font-bold font-mono text-black leading-none">
            {rfp.rfp_number || rfp.id}
          </p>
      ...
      <div className="flex justify-between items-center border border-gray-400 p-1.5 mb-3 bg-gray-50">
        <div className="flex gap-4">
          <span>
            <span className="text-[6pt] uppercase text-gray-600 font-semibold">
              PO Ref:
            </span>{" "}
            <span className="font-mono font-bold">
              {rfp.order_number || rfp.order_id}
            </span>
          </span>
          <span>
            <span className="text-[6pt] uppercase text-gray-600 font-semibold">
            ...
            <span className="font-mono">{rfp.invoiceNumber || "N/A"}</span>
          </span>
        </div>
        <span>
          <span className="text-[6pt] uppercase text-gray-600 font-semibold">
          ...
          <span className="font-bold uppercase">{rfp.payment_method}</span>
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div className="border border-gray-400 p-2">
          <p className="text-[6pt] uppercase text-gray-600 font-semibold border-b border-gray-300 pb-0.5 mb-1">
            Payee
          </p>
          <p className="text-[10pt] font-bold text-black leading-tight">
            {rfp.payable_to}
          </p>
          <p className="text-[7pt] text-gray-700">{rfp.contact_number}</p>
        </div>
        <div className="border border-gray-400 p-2">
          <p className="text-[6pt] uppercase text-gray-600 font-semibold border-b border-gray-300 pb-0.5 mb-1">
            Requestor
      ...
      <div className="grid grid-cols-4 gap-2 mb-3 border border-gray-400 p-2">
        <div>
          <p className="text-[6pt] uppercase text-gray-600 font-semibold">
            Due Date
          </p>
          <p className="text-[9pt] font-bold">{formatDate(rfp.due_date)}</p>
        </div>
        <div>
          <p className="text-[6pt] uppercase text-gray-600 font-semibold">
            Department
        ...
        <div>
          ...
          <p className="text-[9pt] font-bold">{rfp.line_items?.length || 0}</p>
        </div>
        <div className="text-right">
          <p className="text-[6pt] uppercase text-gray-600 font-semibold">
            Amount Due
          </p>
          <p className="text-[12pt] font-bold font-mono text-black">
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-100 border-b border-gray-400">
                <TableHead className="text-[6pt] font-bold text-black uppercase py-1 px-1 text-center w-6">
                  #
                </TableHead>
                <TableHead className="text-[6pt] font-bold text-black uppercase py-1 px-1">
                  Invoice Number
                </TableHead>
                <TableHead className="text-[6pt] font-bold text-black uppercase py-1 px-1 text-center w-8">
                  Qty
                </TableHead>
                <TableHead className="text-[6pt] font-bold text-black uppercase py-1 px-1 text-right w-16">
                  Price
                </TableHead>
                <TableHead className="text-[6pt] font-bold text-black uppercase py-1 px-1 text-right w-16">
                  Amount
                </TableHead>
                <TableHead className="text-[6pt] font-bold text-black uppercase py-1 px-1 w-20">
                  Account
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {rfp.line_items?.map((item, index) => (
                <TableRow
                  key={item.id || index}
                  className="border-b border-gray-200 last:border-b-0"
                >
                  <TableCell className="py-0.5 px-1 text-center text-gray-700">
                    {index + 1}
                  </TableCell>
                  <TableCell className="py-0.5 px-1">
                    <p className="font-semibold text-[8pt] leading-tight">
                      {item.particulars}
                    </p>
                    <p className="text-[6pt] text-gray-600 leading-none">
                  ...
                  <TableCell className="py-0.5 px-1 text-right font-mono font-semibold text-[7pt]">
                    {formatCurrency(item.total_amount)}
                  </TableCell>
                  <TableCell className="py-0.5 px-1 text-[6pt] text-gray-700 leading-tight">
                    {item.charge_to || "-"}
                  </TableCell>
                </TableRow>
              ))}
              ...
              {(!rfp.line_items || rfp.line_items.length === 0) && (
                <TableRow>
                  <TableCell
                    colSpan={6}
                    className="py-2 text-center text-gray-500 text-[7pt]"
                  >
                    No line items
                  </TableCell>
                </TableRow>
            ...
            <TableFooter>
              <TableRow className="bg-gray-100 border-t-2 border-black">
                <TableCell
                  colSpan={2}
                  className="py-1 px-1 text-right font-bold text-[7pt] uppercase"
                >
      ...
      {rfp.journalEntry && rfp.journalEntry.length > 0 && (
        <div className="mb-3">
          <p className="text-[7pt] font-bold text-black uppercase tracking-wider mb-1">
          ...
          {rfp.journalEntry.length > 6 ? (
            <div className="border border-gray-400 p-2">
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[7pt]">
                ...
                <div className="font-bold border-b border-gray-300 pb-0.5 text-right">
                  Amount
                </div>
                {rfp.journalEntry.map((entry) => (
                  <div key={entry.id} className="contents">
                    <div
                      className={
                        entry.entryType === "debit" ? "pl-2" : "pl-2 italic"
                      }
                    >
                      {entry.accountTitle} ({entry.entryType})
                    </div>
                    <div
                      className={`text-right font-mono ${entry.entryType === "debit" ? "" : "italic"}`}
                    >
            ...
            <div className="border border-gray-400">
              <Table>
                <TableHeader>
                  <TableRow className="bg-gray-100 border-b border-gray-400">
                    <TableHead className="text-[6pt] font-bold text-black uppercase py-1 px-1 w-6">
                      #
                    </TableHead>
                    <TableHead className="text-[6pt] font-bold text-black uppercase py-1 px-1">
                      Account Title
                    </TableHead>
                    <TableHead className="text-[6pt] font-bold text-black uppercase py-1 px-1 text-right w-20">
                      Debit
                    </TableHead>
                    <TableHead className="text-[6pt] font-bold text-black uppercase py-1 px-1 text-right w-20">
                      Credit
                ...
                <TableBody>
                  {rfp.journalEntry.map((entry, index) => (
                    ...
                    >
                      ...
                      <TableCell className="py-0.5 px-1 text-[8pt] font-semibold">
                        {entry.accountTitle}
                      </TableCell>
                      <TableCell className="py-0.5 px-1 text-right font-mono text-[7pt]">
                ...
                <TableFooter>
                  <TableRow className="bg-gray-100 border-t border-gray-400">
                    <TableCell
                      colSpan={2}
                      className="py-0.5 px-1 text-right font-bold text-[6pt] uppercase"
                    >
                      Total Distribution
                    </TableCell>
        ...
        <div className="grid grid-cols-3 gap-3">
          {["Prepared By", "Reviewed By", "Approved By"].map((role) => (
            <div key={role} className="border-t border-gray-400 pt-1">
              <p className="text-[6pt] uppercase text-gray-600 font-semibold mb-3">
                {role}
              </p>
              <div className="h-6 border-b border-gray-400 mb-0.5"></div>
              <p className="text-[6pt] text-gray-500 text-center">
                Signature over Printed Name
              </p>
              <p className="text-[5pt] text-gray-400 text-center mt-0.5">
                Date: ___________
              </p>
            </div>
          ))}
      ...
      <div className="border-t border-gray-300 pt-2 mb-2">
        <p className="text-[6pt] text-gray-700 leading-tight">
          <span className="font-bold">Terms:</span> All expenses verified per
          company policy. Cash advances subject to 3-day liquidation.
          Unliquidated amounts deducted from payroll. Finance approval required.
          Document alterations must be initialed.
        </p>
      </div>
      ...
      <div className="mt-auto pt-2 border-t-2 border-black">
        <div className="flex justify-between items-end text-[6pt] text-gray-600">
          <div>
            <p className="font-bold text-black text-[7pt]">
              {COMPANY_INFO.name}
            </p>
          ...
          <div className="text-right">
            ...
            <p className="font-mono text-[5pt]">
              RFP-{rfp.rfp_number || rfp.id}
            </p>
          </div>
        </div>
      </div>
