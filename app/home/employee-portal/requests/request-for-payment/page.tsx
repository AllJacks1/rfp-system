...
async function getServiceOrders(supabase: any, id: string): Promise<Order[]> {
      ...
      *,
      service_requests(
        *,
        ...
        requested_by:users(user_id, first_name, last_name)
      ),
  ...
  const orders = data.filter(
    (so: any) => so.service_requests?.requested_by?.user_id === id,
  );
...
async function getPurchaseOrders(supabase: any, id: string): Promise<Order[]> {
  const { data, error } = await supabase
  ...
  const orders = data.filter(
    (po: any) => po.purchase_requests?.requested_by?.user_id === id,
  );
...
async function getOrders(supabase: any, id: string): Promise<Order[]> {
  const [serviceOrders, purchaseOrders] = await Promise.all([
    getServiceOrders(supabase, id),
    getPurchaseOrders(supabase, id),
  ]);
...
async function getRPFs(supabase: any, requestor: string) {
  const { data, error } = await supabase
...
export default async function RequestForPaymentPage() {
  ...
  const rfps = await getRPFs(supabase, requestor);
